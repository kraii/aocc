...
Test(re_find, match_exists) {
    ...
    cr_assert_eq(re_find(re, str_l("123abc")), 3);
    cr_assert_eq(re_find(re, str_l("123abc123abc")), 3);
    cr_assert_eq(re_find(re, str_l("abc123abc")), 0);
...
Test(re_find, no_match_exists) {
    int re_compile_err = 0;
    pattern *re = re_compile(str_l("[0-9]{2}"), &re_compile_err);

    cr_assert_eq(re_find(re, str_l("abc")), -1);
    cr_assert_eq(re_find(re, str_l("1foot")), -1);
...
Test(re_match, loop_with_get_index) {
    int re_compile_err = 0;
    pattern *re = re_compile(str_l("[0-9]+"), &re_compile_err);
    match_data *md = re_prepare_match(re, str_l("a 123 1 456 789"));
    //                                              012345678901234
    int i = 0;
